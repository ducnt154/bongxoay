...
{	
	...
	if($_POST['subform'] == "1")
	{
		...
		if($user_email == ""){$error = $errors['9'];}
		elseif(!verify_valid_email($user_email)){$error = $errors['10'];}
		else
		{
			...
			if($te > 0){$error = $errors['28'];}
		}		
		
		if($pass != "" || $pass2 != "")
		{
			if($pass == ""){$error = $errors['29'];}
			elseif($pass2 == ""){$error = $errors['30'];}
			else
			{
				if($pass == $pass2)
				{
