			...
			if($uploadedimage != "")
			{
				...
				if($error == "")
				{
					...
					if(file_exists($config['pdir']."/".$thepp))
					{
                       ...
                       $url = $config[postfolder].$pid."/".makeseo($title).".html?new=1";
                       $url = substr($url,20);
						echo $config[postfolder].$pid."/".makeseo($title).".html?new=1";exit;
                       /* if($config[SEO] == 1){echo $config[baseurl].$config[postfolder].$pid."/".makeseo($title).".html?new=1");exit;}
                        else{echo $config[baseurl].$config[postfolder].$pid."/?new=1");exit;}*/
					}
				}
			}
		}
