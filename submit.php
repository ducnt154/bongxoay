...
$SID = $_SESSION['USERID'];
//phân quyền write views
$queryuser = "SELECT * FROM members WHERE USERID ='".$SID."'";

$executequeryuser = $conn->execute($queryuser);

$user =  $executequeryuser->getrows();

foreach($user as $users){

}
STemplate::assign('ad',$users['ad']);
	...
	else
	{
		...
		if($t == "v" && $config['vupload'] == 1)
		{
			...
			if($post_type == "Video")
			{
                ...
                $postviewed = cleanit($_REQUEST['postviewed']);
				...
				if((!strstr($url, 'youtube.com/watch?v=')) && (!strstr($url, 'funnyordie.com/videos/')) &&(!strstr($url, 'dailymotion.com/video/')) && (!strstr($url, 'videofy.me/')) && (!strstr($url, 'vimeo.com/')))
				{
					$error = $errors['23'];
				}
				...
				if($error == "")
				{
                    ...
                    elseif(strstr($url, 'dailymotion.com/video/'))
                    {
                        $dai_url = $url;
                        $position       = strpos($dai_url, 'dailymotion.com/video/')+22;
                        $remove_length  = strlen($dai_url)-$position;
                        $video_id       = substr($dai_url, -$remove_length, 7);
                        $addme = ", dai_key='".mysql_real_escape_string($video_id)."'";
                    }
        ...
        elseif($t == "t" && $config['tupload'] == 1)
        {
            ...
            if($post_type == "Text")
            {
                ...
                if($title == "")
                {
                    $error = "B?n ph?i nh?p tiêu ?? bài vi?t";
                }
                elseif($contents == "")
                {
                    $error = "B?n ph?i nh?p n?i dung bài vi?t";
                }
                elseif($url == "")
                {
                    $error = $errors['26'];
                                    ...
                                    if($uploadedimage != "")
                                    {
                                        ...
                                        if($error == "")
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                //the frontend picture of gif
                                                $im=imagecreatefromgif($myvideoimgnew);
                                                imagejpeg($im, $config['pdir']."/t/".$pid.".jpg");
                                                imagedestroy($im);
